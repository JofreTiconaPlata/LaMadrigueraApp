...
const handleIngresoError = (error: unknown, res: Response): void => {
  ...
  const errorMap: Record<string, { status: number; message: string }> = {
    ...
    RESERVA_NOT_FOUND: { status: 404, message: 'Reserva no encontrada' },
    RESERVA_NOT_ACTIVE: { status: 409, message: 'La reserva no está activa' },
    RESERVA_NOT_MATCH: { status: 400, message: 'La reserva no coincide con el parqueo, espacio o vehículo indicado' },
    RESERVA_ALREADY_USED: { status: 409, message: 'La reserva ya fue usada para registrar un ingreso' },
